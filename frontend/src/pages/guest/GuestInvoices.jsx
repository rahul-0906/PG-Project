...
export default function GuestInvoices() {
  ...
  const handlePayNow = async (invoice) => {
    ...
    try {
      ...
      const orderRes = await api.post(`/api/guest/invoices/${invoice.id}/initiate-payment`);
      ...
      if (order.mock) {
        ...
        if (window.confirm(`[DEV MODE] Simulate payment of ₹${invoice.totalAmount} for ${MONTHS[invoice.month-1]} ${invoice.year}?`)) {
          await api.post(`/api/guest/invoices/${invoice.id}/verify-payment`, {
        ...
        <div className="card" style={{ padding: 0, overflow:'hidden' }}>
          ...
          {invoices.length === 0 && (
            <div style={{ padding:'3rem', textAlign:'center', color:'var(--text-muted)' }}>
              📄 No invoices yet. They are generated monthly by the billing scheduler.
            </div>
          )}
              ...
              <div>
                <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap' }}>
                  {inv.status !== 'PAID' ? (
                    ...
                    <span style={{ color:'#10b981', fontSize:'0.8rem', fontWeight:700, padding:'0.4rem 0' }}>✅ Paid</span>
                  )}
