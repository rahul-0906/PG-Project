    ...
    <div className="layout">
      ...
      <div className="main-content fade-in">
        <div className="page-header">
          <div><h1 className="page-title">EB Bill ⚡</h1><p className="page-subtitle">Record electricity bills per block</p></div>
        </div>
            ...
            {mode === 'METER_BASED' && (
              <div style={{ marginTop: '1rem', marginBottom: '1.5rem', background: 'var(--bg-secondary)', padding: '1rem', borderRadius: 8 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
                  ...
                  <button type="button" className="btn btn-secondary" style={{ padding: '0.2rem 0.6rem', fontSize: '0.8rem' }} onClick={addReadingRow}>+ Add Guest</button>
                </div>
                ...
                {readings.map((r, i) => (
                  <div key={i} style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr auto', gap: '0.5rem', marginBottom: '0.5rem', alignItems: 'end' }}>
                    ...
                    <button type="button" className="btn btn-ghost" style={{ color: 'var(--danger)', padding: '0.5rem' }} onClick={() => removeReadingRow(i)}>✖</button>
                  </div>
                ))}
              </div>
            )}

            <button type="submit" className="btn btn-primary" disabled={saving}>{saving?'Processing...':'⚡ Split & Save'}</button>
          </form>
          {result && (
            <div style={{ marginTop:'1.5rem', padding:'1rem', background:'rgba(16,185,129,0.1)', borderRadius:10, border:'1px solid var(--success)' }}>
              <div style={{ color:'var(--success)', fontWeight:700, marginBottom:'0.5rem' }}>✅ EB Bill Recorded</div>
